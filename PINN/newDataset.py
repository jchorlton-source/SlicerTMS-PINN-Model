...
def make_dataloaders(sim_output_dir, batch_size=4, num_workers=8,
                     split_ratio=0.8, train_fraction=1,
    ...
    if train_fraction < 1.0:
        ...
        n_keep = int(len(train_set)*train_fraction)
        rng = torch.Generator().manual_seed(123)
        indices = torch.randperm(len(train_set), generator=rng).tolist()[:n_keep]
        train_set = Subset(train_set, indices)
        if rank == 0:
            print(f"[INFO] Subsampled train set to {n_keep} samples "
                  f"({train_fraction*100:.0f}% of full train split)")
