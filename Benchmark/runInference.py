...
CKPT    = os.path.join(SCRATCH, 'benchmark/PINN/best_model_ema.pth')
OUT_DIR = os.path.join(SCRATCH, 'benchmark/sanity_check_PINN1')
os.makedirs(OUT_DIR, exist_ok=True)

device = torch.device('cuda:0')

# --- Build model and load EMA weights ---
model = DualBranchResNet3D(base_channels=32).to(device)
...
print(f"Selected validation sample index: {idx} of {len(val_dataset)}")
(dadt, cond), target = val_dataset[idx]

# Add batch dimension since we're bypassing the loader
dadt   = dadt.unsqueeze(0).to(device)
...
vmin = min(target_mag.min(), pred_mag.min())

for col, sl in enumerate(slices):
    axes[0, col].imshow(target_mag[sl], cmap='plasma', vmin=vmin, vmax=vmax)
    axes[0, col].set_title(f"Target |E|, z={sl}")
    axes[1, col].imshow(pred_mag[sl],   cmap='plasma', vmin=vmin, vmax=vmax)
    axes[1, col].set_title(f"Pred |E|, z={sl}")
    axes[2, col].imshow(diff_mag[sl],   cmap='plasma', vmin=vmin * 0.5, vmax=vmax * 0.5)
