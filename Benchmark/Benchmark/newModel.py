...
def gn(num_channels, groups=8):
    ...
    return nn.GroupNorm(num_groups=min(groups, num_channels),
                        num_channels=num_channels, affine=True)
...
class Encoder3D(nn.Module):
    def __init__(self, in_channels, base_channels=32):
        ...
        self.stem = nn.Sequential(
            nn.Conv3d(in_channels, base_channels, kernel_size=3,
                      padding=1, bias=False),
            gn(base_channels),
...
class Decoder3D(nn.Module):
    def __init__(self, base_channels=32):
        ...
        self.up1 = nn.Sequential(
            ...
            nn.Conv3d(base_channels * 16, base_channels * 8, kernel_size=3,
                      padding=1, bias=False),
            gn(base_channels * 8),
            nn.ReLU(inplace=True),
        )
        self.up2 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False),
            nn.Conv3d(base_channels * 16, base_channels * 4, kernel_size=3,
                      padding=1, bias=False),
            gn(base_channels * 4),
            nn.ReLU(inplace=True),
        )
        self.up3 = nn.Sequential(
            nn.Upsample(scale_factor=2, mode='trilinear', align_corners=False),
            nn.Conv3d(base_channels * 8, base_channels * 2, kernel_size=3,
                      padding=1, bias=False),
            gn(base_channels * 2),
            nn.ReLU(inplace=True),
        )
        self.up4 = nn.Sequential(
            nn.Conv3d(base_channels * 4, base_channels, kernel_size=3,
                      padding=1, bias=False),
            gn(base_channels),
            nn.ReLU(inplace=True),
        )
        # final 1x1x1: no norm, no activation — keep raw scale
