...
## root setting
train_path = '/NAS/Synology/MultiCoilData/BrainCSF/Iso/2mm/Code/training_R01/'
model_save_path = '/NAS/Synology/MultiCoilData/BrainCSF/Iso/2mm/Code/GP_data/code/Network_train_toy/models/Iso_threecoils_noinitial/'
...
# load the checkpoint
args.load_model = True # False


if args.load_model:
    print('load previous model--------------')
    model_path = os.path.join(model_save_path, 'Trained_isocond_epoch_600.pth.tar')
...
def train_model(trainloader, net, criterion, optimizer, 
    ...
    file_loss_train = open('models/Iso_threecoils_noinitial/infor_train_isocond_epoch200.txt', 'a')
    file_epoch_time = open('models/Iso_threecoils_noinitial/epoch_time.txt', 'a')  
    # record time
    # write starting time
    ...
    num_steps = 0
    # save log
    writer = SummaryWriter(log_dir="log_isocond_continue")   
    num_steps = 0
...
#            print('Used time in %d iterations: %.2f\n' % (i+1, one_it_time_end))
            ...
            if (i+1) % 10 == 0:    # print every 2 mini-batches
                print('[Epoch: %d/%d, Batch index: %5d] loss: %.3f' %
                      (epoch + 1, num_epochs, i + 1, running_loss*10000))
...
#                writer.add_scalars('dice', {'train_dice_mean': train_dice_mean}, num_steps)  
            ...
            # save model after 10000 iters
            if num_steps % 10000 == 0 and args.local_rank == 0:
        ...
        if (epoch+1) % 100 == 0 or epoch == (num_epochs-1):
            if args.local_rank == 0 and flag_dist == 0:
                ...
                model_save_name = "Trained_isocond_epoch_%d.pth.tar" % (epoch + 1)
                model_save = os.path.join(model_save_path, model_save_name)
                torch.save({
                'epoch': epoch,
...
f = open('models/Iso_threecoils_noinitial/infor_recored.txt', 'w')
