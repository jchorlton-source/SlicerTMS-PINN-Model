...
model_fold = 'model_save/3d-unet_epoch300_t1_norm.pt'
...
#        ])
dataset_train = LDM.RandomSamples(root_path)
train_loader = DataLoader(dataset_train, batch_size=batch_size,
                              shuffle=True)
# load a subject, check a subject
# Note: here the batch is dict
subj, simu = next(iter(train_loader))
print("Sample size: ", subj.size())
print("Target size:", simu.size())
...
out_channels = 3
base_n_filter = 16
net = Baseline3DUNet(in_channels, out_channels, base_n_filter) 
net = net.float()
#print(net)
...
def train_model(trainloader, model, criterion, optimizer, scheduler, num_epochs=25):
    ...
    num_steps = 0
    # save log
    writer = SummaryWriter(log_dir="log_T1_2")    
    for epoch in tqdm(range(num_epochs)):
        ...
        for i, data in enumerate(trainloader, 0): # index = 0
            ...
            if (i+1) % 1 == 0:    # print every 2000 mini-batches
                print('[Epoch: %d/%d, Batch index: %5d] loss: %.3f' %
                      (epoch + 1, num_epochs, i + 1, running_loss/1))
                running_loss = 0.0                
        ...
        if epoch % 9 == 0 or epoch == (num_epochs-1):
            print('[saving model]-[INFO]:Epoch-', epoch + 1)
...
#net.eval()
dataset_test = RandomSamples(test_path, 'testing/t1d', 'simu_target', 
...
for i, data in enumerate(testloader):
    ...
    save_pred_path = os.path.join(test_path, 'simu_pred_T1_Norm')
    save_subj_name = os.path.join(save_pred_path, ('%s_E_pd.mat' % (subj_name)))
    # save it as mat
    scipy.io.savemat(save_subj_name, dict(pred=pd_out_save))
    # save it as narray
#    np.save(save_subj_name, pd_out_save) 
    # save it as nifti
#    array_img = nib.Nifti1Image(pd_out_save, affine=eye())
#    nib.save(pd_out_save, 'nifti.nii.gz')
