...
model_save_path = '/NAS/Synology/MultiCoilData/BrainCSF/Iso/2mm/Code/GP_data/code/Network_train_toy/models/Iso_wMSE_noinitial/'
...
if args.load_model:
    ...
    model_path = os.path.join(model_save_path, 'MultiCoil_MSE_epoch_900.pth.tar')
...
def train_model(trainloader, net, criterion, optimizer, 
    ...
    file_loss_train = open('models/Iso_wMSE_noinitial/infor_wMSE_epoch200.txt', 'a')
    file_epoch_time = open('models/Iso_wMSE_noinitial/epoch_time.txt', 'a')  
    # record time
    # write starting time
    ...
    num_steps = 0
    # save log
    writer = SummaryWriter(log_dir="log_wMSE_continue")   
    num_steps = 0
...
#                writer.add_scalars('dice', {'train_dice_mean': train_dice_mean}, num_steps)  
        ...
        if (epoch+1) % 50 == 0 or epoch == (num_epochs-1):
            if args.local_rank == 0 and flag_dist == 0:
                ...
                model_save_name = "Trained_wMSE_epoch_%d.pth.tar" % (epoch + 1)
                model_save = os.path.join(model_save_path, model_save_name)
                torch.save({
                'epoch': epoch,
