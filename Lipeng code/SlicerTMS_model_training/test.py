...
root_path = '/rfanfs/pnl-zorro/home/lipeng/NAS/QNAP/MultiCoilData/AnisoCondData/GP_data/train_toy'
valid_path = '/rfanfs/pnl-zorro/home/lipeng/NAS/QNAP/MultiCoilData/AnisoCondData/GP_data/validation'

train_path = os.path.join(root_path, 'normalized')
test_path = os.path.join(root_path, 'testing_data')
val_path = os.path.join(valid_path, 'normalized')
model_save_path = os.path.join('/rfanfs/pnl-zorro/home/lipeng/NAS/QNAP/MultiCoilData/AnisoCondData/GP_data/code/Network_train_toy/models/normalized')
...
if args.load_model:
    ...
    model_path = os.path.join(model_save_path, 'model_subj20_cond_res_epoch_1_pre.pth.tar')
...
def train_model(trainloader, val_loader, net, criterion, optimizer, 
                scheduler, num_epochs=25, batch_size=4, next_epoch=0):
    # save infor in toe aach file
    file_loss_train = open('model_save_norm/infor_train_loss_20subj_res_cond.txt', 'a')
    file_loss_val = open('model_save_norm/infor_val_loss_20subj_res_cond.txt', 'a')  
    file_epoch_time = open('model_save_norm/epoch_time.txt', 'a') 
    # record time
    # write starting time
    ...
    num_steps = 0
    # save log
    writer = SummaryWriter(log_dir="log_20subj_cond_resunet")   
    num_steps = 0
...
#                        writer.add_scalars('dice', {'val_dice_mean': val_dice_mean}, num_steps)
            ...
            if num_steps % 5000 == 0 and args.local_rank == 0:
                model_save_name = "model_subj20_cond_res_iter_%d.pth" % (num_steps)
        ...
        if (epoch+1) % 1 == 0 or epoch == (num_epochs-1):
            if args.local_rank == 0 and flag_dist == 0:
                ...
                model_save_name = "model_subj20_cond_res_epoch_%d.pth.tar" % (epoch + 1)
                model_save = os.path.join(model_save_path, model_save_name)
                torch.save({
                'epoch': epoch,
    ...
    return net, time_elapsed, mean_loss_after_period
    
####
## do training  
## train and save the model path
print('#'*10, 'Training', '#'*10)
net, time_elapsed, mean_loss_after_period = train_model(train_loader, val_loader, net, criterion, 
        optimizer, exp_lr_scheduler, num_epochs=num_epochs, batch_size=batch_size, next_epoch=next_epoch)
...
f = open('model_save_norm/infor_recored.txt', 'w')
