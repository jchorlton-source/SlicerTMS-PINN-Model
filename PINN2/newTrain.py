...
from PhysicsLoss import hybrid
...
# -----------------------------
def relative_l2_components(pred, target, mask=None, auto_mask_threshold=1e-6):
    if mask is None:
        mag = target.abs().sum(dim=1, keepdim=True)
        mask = (mag > auto_mask_threshold).to(pred.dtype)
    pred   = pred * mask
    target = target * mask
...
class CompositeLoss(nn.Module):
    ...
    def forward(self, pred, target, mask: torch.Tensor | None = None, **_unused):
        if mask is None:
...
def train_one_epoch(model, loader, loss_fn, optimizer, device,
    ...
    # Per-component accumulators for PINN diagnostics. Populated only if
    # the loss exposes a `last_components` dict (PhysicsInformedLoss does;
    # CompositeLoss does not — in that case these stay zero).
    comp_sums = {
        'data':       torch.zeros((), device=device, dtype=torch.float64),
        'continuity': torch.zeros((), device=device, dtype=torch.float64),
        'curl':       torch.zeros((), device=device, dtype=torch.float64),
    }
    ...
    for batch in iterator:
        ...
        with amp_ctx:
            out  = model(dadt, cond)
            loss = loss_fn(out, target, dadt=dadt, conductivity=cond, mask=mask)
        ...
        # Accumulate physics components on every rank — required for the
        # all_reduce below to produce the correct global mean.
        if hasattr(loss_fn, 'last_components'):
            for k, v in loss_fn.last_components.items():
                if k in comp_sums:
                    comp_sums[k] += v.double() * bs

    # Global mean across all ranks
    dist.all_reduce(loss_sum,     op=dist.ReduceOp.SUM)
    dist.all_reduce(sample_count, op=dist.ReduceOp.SUM)
    for k in comp_sums:
        dist.all_reduce(comp_sums[k], op=dist.ReduceOp.SUM)

    train_loss = (loss_sum / sample_count).item()
    comp_means = {k: (v / sample_count).item() for k, v in comp_sums.items()}
    return train_loss, comp_means
...
def train_model(
    ...
    loss_fn = hybrid(w_data=1.0, w_continuity=0.05, w_curl=0.1, w_bg=0.15, dx=1.0).to(device)
    ...
    for epoch in range(start_epoch, epochs + 1):
        ...
        if is_master:
            print(f"  Train Loss: {train_loss:.6f} | "
                  ...
                  f"Rel L2: {rel_l2:.6e} | "
                  f"data: {train_comps['data']:.4e} | "
                  f"cont: {train_comps['continuity']:.4e} | "
                  f"curl: {train_comps['curl']:.4e} | "
            ...
            history["data"].append(train_comps['data'])
            history["continuity"].append(train_comps['continuity'])
            history["curl"].append(train_comps['curl'])

            csv_path = os.path.join(checkpoint_dir, "training_log.csv")
            with open(csv_path, "w", newline="") as f:
                writer = csv.writer(f)
                writer.writerow(["epoch", "train_loss", "val_loss",
                                 "rel_l2", "data", "continuity", "curl",
                                 "epoch_time", "lr"])
...
if __name__ == "__main__":
    ...
    ckpt_dir = os.path.join(base, 'PINN_hybrid_small')
    ...
    train_model(
        ...
        plateau_epochs = 95,       # epochs 6-50 at peak LR
        base_channels  = 32,
        ema_decay      = 0.999,
